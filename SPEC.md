    ...
    spec:
      ...
      backend:
        ...
        s3:
          enabled: bool
          secretRef: string
          configMapRef: string         # optional ConfigMap for bucket/region/endpoint metadata
          autoDetectOBC: bool          # auto-detect OBC secret/configmap and default key names (default true)
          endpoint: string             # inline endpoint URL (non-sensitive)
          endpointKey: string
          endpointHostKey: string      # optional host key when endpoint is split across host/port
          endpointPortKey: string
          endpointScheme: string       # "http" | "https" (defaults to http)
          emitS3EndpointUrl: bool       # optionally emit S3_ENDPOINT_URL alias
          accessKeyIdKey: string
          secretAccessKeyKey: string
          bucket: string               # inline bucket name (non-sensitive)
          bucketKey: string
          region: string               # inline AWS region (non-sensitive)
          regionKey: string
      dashboard:
        enabled: bool
...
### M1: Project Bootstrap & CRD Design
  ...
  - [x] Add unit tests for config/secrets rendering helpers, ensuring env var construction matches Convex expectations.
  ...
  - [x] Integrate golangci-lint, go fmt, go vet, and make test targets into CI (GitHub Actions or equivalent), with caching to speed runs.
  - [x] Write developer guide for local kind/minikube deployment, including how to apply sample dev/prod ConvexInstance manifests and verify readiness via kubectl and curl against Gateway host (with example
    values).
