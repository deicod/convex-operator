...
- `spec.backend`: image (defaults to Convex backend), `db.engine` (`postgres|mysql|sqlite`) plus Secret/key refs and optional `databaseName` override for `INSTANCE_NAME` (defaults to the instance name with `-` replaced by `_`), `storage.mode` (`sqlite|external`) with optional PVC, S3 secret/key wiring (`endpointKey`, `regionKey`, `accessKeyIdKey`, `secretAccessKeyKey`, `bucketKey`), and optional `security` overrides for pod/container contexts (defaults defer to the image user, avoiding forced `runAsNonRoot`).
...
- Backend pods receive `CONVEX_PORT` (3210), `CONVEX_ENV` (spec.environment), `CONVEX_VERSION` (spec.version), `INSTANCE_NAME` (defaults to the ConvexInstance name with `-` -> `_`, override via `spec.backend.db.databaseName`), plus secret-derived `CONVEX_ADMIN_KEY`/`CONVEX_INSTANCE_SECRET`, `POSTGRES_URL` (or `MYSQL_URL` when `spec.backend.db.engine: mysql`) from the referenced `urlKey`, and S3 wiring (`AWS_REGION`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, optional `AWS_ENDPOINT_URL`/`AWS_ENDPOINT_URL_S3`, `S3_STORAGE_*` buckets) when enabled. If set in the spec, `spec.networking.cloudOrigin` and `spec.networking.siteOrigin` populate `CONVEX_CLOUD_ORIGIN`/`CONVEX_SITE_ORIGIN`; otherwise they default to the external host (`scheme://spec.networking.host`).
