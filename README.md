...
- **Custom Gateway:** Use `parentRefs` to attach the HTTPRoute to an existing Gateway instead of creating one (TLS/listeners on that Gateway are managed externally).
- **ListenerSet:** Use `listenerSet.parentGateway` to have the operator manage this instance's listener (hostname + TLS) on a shared Gateway via a Gateway API `ListenerSet`, without patching the Gateway or provisioning a dedicated one. Requires Gateway API 1.5+ and an implementation that supports `ListenerSet` (e.g. NGINX Gateway Fabric 2.6+). Takes precedence over `parentRefs`.
...
- Set `spec.networking.listenerSet.parentGateway` to have the operator manage the instance's listener (hostname + TLS) on a shared Gateway via a Gateway API `ListenerSet`, instead of patching that Gateway or provisioning a dedicated one. The shared Gateway must permit attachment from the instance namespace through its `allowedListeners` field; the operator never mutates the shared Gateway. The HTTPRoute attaches to the operator-managed `ListenerSet`. This requires the standard `gateway.networking.k8s.io/v1` `ListenerSet` CRD (Gateway API 1.5+); the operator auto-detects it at startup and reports `GatewayReady=False` with reason `ListenerSetCRDMissing` when it is absent. `listenerSet` takes precedence over `parentRefs`.
- The controller is built against Gateway API `v1.5.1`. The `Gateway`/`HTTPRoute` modes only use GA `gateway.networking.k8s.io/v1` fields present in the `1.3.x`, `1.4.x`, and `1.5.x` standard CRD bundles, so they keep working on 1.3/1.4 clusters; the optional `ListenerSet` mode additionally requires the `1.5+` standard `ListenerSet` CRD. Use `make test-gateway-api-compat` to run the envtest suite against all three CRD versions (ListenerSet specs skip automatically when the CRD is absent).
- This repository does not install NGINX Gateway Fabric. If you upgrade NGF separately to `v2.4.2`, use the upstream Gateway API `v1.4.x` install and migrate any Helm/config usage of `snippetsFilters` to `snippets`. The `ListenerSet` networking mode additionally requires NGF `v2.6.0+` with the Gateway API `v1.5+` standard CRDs installed.
